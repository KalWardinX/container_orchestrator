...
handle_run() {
    ...
    mkdir -p "$NEW_LAYER/diff"
    mkdir -p "$NEW_LAYER/work"
    mkdir -p "$NEW_LAYER/merged"

    # Subtask 3.f.2
    # Temporarily mount the overlay filesystem
    mount -t overlay -o lowerdir="$parent_layers",upperdir="$NEW_LAYER/diff",workdir="$NEW_LAYER/work" none "$NEW_LAYER/merged"

    # Subtask 3.f.3
    # Execute the command in the new mount
    chroot "$NEW_LAYER/merged" /bin/bash -c "$command"
    
    # Subtask 3.f.4
    # Cleanup and record metadata
    umount "$NEW_LAYER/merged"
...
handle_copy() {
    ...
    cp -r "$src" "$NEW_LAYER/merged/$dest"
...
run() {
    ...
    mkdir -p "$CONTAINERDIR/$NAME/rootfs"
    local LAYER_STACK=$(cat "$IMAGEDIR/$IMAGE/layers")
    local MERGED_LAYER="$CONTAINERDIR/$NAME/rootfs"
    local WORKDIR="$CONTAINERDIR/$NAME/work"
    local UPPER_LAYER="$CONTAINERDIR/$NAME/diff"

    mkdir -p "$WORKDIR" "$UPPER_LAYER"
    mount -t overlay -o lowerdir="$LAYER_STACK",upperdir="$UPPER_LAYER",workdir="$WORKDIR" none "$MERGED_LAYER"
    ...
    # if [ $? -ne 0 ]; then
    #     echo -e "Failed to bind mount /dev"
    #     exit 1
    # fi

    # Subtask 3.d.3
    # Modify subtask 3.a.1 to bind mount /dev
        mkdir -p "$MERGED_LAYER/dev"
    mkdir -p "$MERGED_LAYER/dev/pts"

    mount --bind /dev "$MERGED_LAYER/dev"
    mount -t devpts devpts "$MERGED_LAYER/dev/pts"
    ...
    # unshare -fuipnm --kill-child  --mount-proc chroot $CONTAINERDIR/$NAME/rootfs /bin/bash -c "
    ...
    unshare -fuipnm --kill-child  --mount-proc chroot "$MERGED_LAYER" /bin/bash -c "
        mount -t proc proc /proc
        mount -t sysfs sys /sys
        chmod 755 /
        exec $INIT_CMD_ARGS
...
stop() {
    ...
    # Subtask 3.d.4
    # Unmount the overlay filesystem
    umount "$CONTAINERDIR/$NAME/rootfs/dev/pts" > /dev/null 2>&1 || :
    umount "$CONTAINERDIR/$NAME/rootfs/dev" > /dev/null 2>&1 || :
...
exec() {
    ...
    nsenter -a -t "$CONTAINER_INIT_PID" -r --wd /bin/bash -c "
        ls /
        pwd
        mount -t proc proc /proc
        $EXEC_CMD_ARGS
...
addnetwork() {
    ...
    # local PID=$(ps -ef | grep "$CONTAINERDIR/$NAME/rootfs" | grep -v grep | awk '{print $2}')
    local MERGED_LAYER="$CONTAINERDIR/$NAME/rootfs"
    local PID=$(ps -ef | grep "$MERGED_LAYER" | grep -v grep | awk '{print $2}')
